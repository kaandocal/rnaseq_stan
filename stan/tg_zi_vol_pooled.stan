...
parameters {
    ...
    vector<lower=0,upper=1>[N] raw_betas_1;
    vector<lower=0,upper=1>[N] raw_betas_2;
...
model {
    ...
    raw_betas_1 ~ beta(sigma_on[genes], sigma_off[genes]);
    raw_betas_2 ~ beta(sigma_on[genes], sigma_off[genes]);
    ...
    for (n in 1:N) {
        ...
        real beta = raw_betas_1[n] + raw_betas_2[n];

        if (counts[n] == 0) {
            target += log_sum_exp(bernoulli_lpmf(1 | p0[g]), bernoulli_lpmf(0 | p0[g]) + poisson_lpmf(counts[n] | rho_eff * beta));
        } else {
            target += bernoulli_lpmf(0 | p0[g]) + poisson_lpmf(counts[n] | rho_eff * beta);
        }
    }
}
